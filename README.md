...
第一次启动会不成功，但是只要nocas和数据库正常启动了就行。需要更改许多nocas上的配置文件，主要是服务的ip地址和访问密码。之后再重启一次，服务就能正常起来了。千万不要按照教程直接把初始化的sql脚本放到/data/docker/mysql/data下，会导致数据库第一次起不起来。这个数据可以不导入，不影响使用。只需要init.sql就可以了。
